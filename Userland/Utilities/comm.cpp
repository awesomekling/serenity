...
int main(int argc, char** argv)
{
    ...
    if (file1_path == "-" && file2_path == "-") {
        warnln("File1 and file2 cannot both be STDIN");
        return 1;
    }

    RefPtr<Core::File> file1, file2;
    if (file1_path == "-") {
        file1 = Core::File::standard_input();
    } else {
        auto file1_or_error = Core::File::open(file1_path, Core::OpenMode::ReadOnly);
        if (file1_or_error.is_error()) {
            warnln("Failed to open file1 '{}': {}", file1_path, file1_or_error.error());
            return 1;
        }
        file1 = file1_or_error.value();
    }
    if (file2_path == "-") {
        file2 = Core::File::standard_input();
    } else {
        auto file2_or_error = Core::File::open(file2_path, Core::OpenMode::ReadOnly);
        if (file2_or_error.is_error()) {
            warnln("Failed to open file2 '{}': {}", file2_path, file2_or_error.error());
            return 1;
        }
        file2 = file2_or_error.value();
    }
    ...
    int indent_level { 0 };
    if (!suppress_col1)
        col1_fmt = String::formatted("{}{}", tabs[indent_level++], print_color ? COL1_COLOR : "{}");
    if (!suppress_col2)
        col2_fmt = String::formatted("{}{}", tabs[indent_level++], print_color ? COL2_COLOR : "{}");
    if (!suppress_col3)
        col3_fmt = String::formatted("{}{}", tabs[indent_level++], print_color ? COL3_COLOR : "{}");
