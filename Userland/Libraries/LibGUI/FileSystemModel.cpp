...
void FileSystemModel::handle_file_event(Core::FileWatcherEvent const& event)
{
    ...
    case Core::FileWatcherEvent::Type::ChildCreated: {
        ...
        auto& child_name = parts.last();

        // FIXME: let LexicalPath give us the parent of a path.
        auto parent_name = event.event_path.substring(0, event.event_path.length() - child_name.length() - 1);
