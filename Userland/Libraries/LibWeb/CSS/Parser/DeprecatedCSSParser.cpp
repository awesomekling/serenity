...
static CSS::CalculatedStyleValue::CalcSum parse_calc_expression(const StringView& expression_string)
{
    // First, tokenize
...
RefPtr<CSS::StyleValue> parse_css_value(const CSS::ParsingContext& context, const StringView& string, CSS::PropertyID property_id)
{
    ...
    if (string.starts_with("calc(")) {
        ...
        if (calc_expression.has_value())
            return CSS::CalculatedStyleValue::create(calc_expression_string, calc_expression.value());
    }
